 ...
 * actually clear is stack + gap + keyboard, not just stack + gap, so the
 * last row(s) stayed permanently behind the capsule row no matter how far
 * the list scrolled — proven on-device with a short (6-item) filtered list
 * where the crop above the search bar was byte-identical from the very
 * first render through 15 scroll swipes (there was no scroll distance left
 * to reach, because the missing term meant the content was already as tall
 * as it would ever get). See `useKeyboardLayoutAnimation.test.ts`.
 *
...
export function computeSearchStackBottomPadding({
  ...
  return (stackHeight || fallbackHeight) + gap + Math.max(0, keyboardHeight);
}
